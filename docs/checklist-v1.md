...
| Conferidor de assinaturas (integridade PAdES no navegador) | Feito | `src/tool/lib/assinatura.ts`, `politicas-icp.ts`, ilha em `src/tool/conferidor/`, página `/verificar-assinatura-digital/`; Etapa 0 em `docs/conferidor-etapa-0.md` |
...
| Revogação (LCR/OCSP) | Fora do escopo por decisão | Consultar revogação contaria a um terceiro que o documento está sendo conferido; a interface e `/verificar-assinatura-digital/` dizem isso explicitamente |
